#!/usr/bin/env python3
"""
Pasarguard Node Manager Bot - Otomatik IP Değiştirme ve Node Yönetimi
Tek dosya, tüm özellikler içinde!
"""
...
# ============================================
# ⚙️ KONFIGÜRASYON - BURAYI DÜZENLEYİN!
# ============================================

# Telegram Bot Token (https://t.me/BotFather)
BOT_TOKEN = "8850038202:AAGkr_QlbJJxSAvHR20-zLpWFYUSrYcwa0U"  # 🔴 KENDİ TOKEN'INIZ

# Pasarguard Panel Bilgileri
PANEL_URL = "https://crc.fastline-tm-belet-film.ru:8000"  # 🔴 PANEL URL'İNİZ
ADMIN_USERNAME = "komutan31"  # 🔴 KULLANICI ADINIZ
ADMIN_PASSWORD = "KomutanPanel_13"  # 🔴 ŞİFRENİZ
...
# Telegram Kullanıcı ID'leri (virgülle ayırın - @userinfobot'dan öğrenin)
ALLOWED_USERS = "8359722718 ,7115611768"  # 🔴 KENDİ ID'LERİNİZ
NOTIFY_USERS = "8359722718 ,7115611768"  # 🔴 BİLDİRİM GÖNDERİLECEK ID'LER

# Bot Ayarları
CHECK_INTERVAL = 30  # Kontrol aralığı (saniye)
MIN_TRAFFIC = 1000  # Minimum trafik (bytes/saniye) - 1000 = 1KB/s
...
TRAFFIC_ALERT = 5000  # Trafik uyarı eşiği (5KB/s altında uyar)

# ============================================
# 🚀 BOT KODU - AŞAĞIYI KARIŞTIRMAYIN!
# ============================================
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("node_manager.log"),
...
class PasarguardAPI:
    """Pasarguard API ile iletişim sınıfı"""
    ...
    def __init__(self, base_url: str, username: str, password: str):
        ...
        self.session.verify = False  # SSL sertifikası varsa True yapın
        
    def _get_token(self) -> str:
        """Pasarguard'dan Bearer Token al"""
        if self.token and time.time() < self.token_expiry:
            return self.token
        
        try:
            # Önce /api/auth/token dene
            ...
            logger.info("✅ Token başarıyla alındı (alternatif endpoint)")
            return self.token
    ...
    def get_hosts(self) -> dict:
        """Mevcut host bilgilerini al"""
        return self._request("GET", "hosts")

    def update_host_ip(self, ip: str) -> dict:
        """Host IP'sini güncelle"""
...
# API'yi başlat
api = PasarguardAPI(PANEL_URL, ADMIN_USERNAME, ADMIN_PASSWORD)
...
def get_next_node(state: dict) -> dict:
    """Sıradaki sağlıklı node'u bul"""
    ...
    # Hiç sağlıklı node yoksa sıradakine geç
    next_idx = (current_idx + 1) % len(nodes)
    return nodes[next_idx]

def switch_to_node(node_id: int, state: dict, force: bool = False) -> Tuple[bool, str, dict]:
    """Node'a geçiş yap"""
    nodes = state.get("nodes", NODES)
    ...
    if not target_node:
        return False, "❌ Node bulunamadı!", None
    
    # Zorunlu değilse sağlık kontrolü yap
    if not force:
        is_healthy, info, status = check_node_health(node_id)
        if not is_healthy:
            return False, f"⚠️ Node sağlıksız! {status}", info
    
    try:
        # IP'yi güncelle
        result = api.update_host_ip(target_node["ip"])
        
        if not result:
            return False, "❌ API yanıt vermedi!", None
        ...
        # Başarılı mesajı
        msg = f"✅ {target_node['name']} ({target_node['ip']}) node'una geçildi!"
        return True, msg, None
        
    except Exception as e:
        logger.error(f"Node {node_id} geçiş başarısız: {e}")
        return False, f"❌ Geçiş başarısız: {str(e)}", None
...
async def auto_failover(context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["bad_count"] >= FAIL_COUNT:
        ...
        if next_node and next_node["id"] != active_id:
            success, message, _ = switch_to_node(next_node["id"], state, force=False)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        status_text = f"⚠️ API bağlantısı başarısız: {str(e)}"
...
async def switch_next(q, state: dict):
    ...
    if next_node["id"] == state.get("active_node_id"):
        await safe_edit(q, "ℹ️ Sıradaki node aktif node ile aynı. Tüm node'lar sağlıksız olabilir.", reply_markup=menu_kb(state))
        return
    
    success, message, _ = switch_to_node(next_node["id"], state, force=True)
...
async def on_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success, message, _ = switch_to_node(node_id, state, force=True)
    await safe_edit(q, message, reply_markup=menu_kb(state))

async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Metin komutları işleyici"""
    if not is_allowed(update.effective_user.id):
        return
...
def main():
    ...
    print("=" * 60)
    print("🚀 Pasarguard Node Manager Bot Başlatılıyor...")
    print("=" * 60)
    ...
    try:
        if api.test_connection():
            ...
            print("⚠️ API bağlantısı başarısız, lütfen bilgileri kontrol edin!")
    except Exception as e:
